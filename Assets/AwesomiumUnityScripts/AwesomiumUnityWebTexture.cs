...
public class AwesomiumUnityWebTexture : MonoBehaviour 
{
	...
	public void Initialize()
	{
		...
		else if (this.gameObject.GetComponent<Renderer>() && this.gameObject.GetComponent<Renderer>().material)
		{		
			this.gameObject.GetComponent<Renderer>().material.mainTexture = m_Texture;
			this.gameObject.GetComponent<Renderer>().material.mainTextureScale = new Vector2(	Mathf.Abs(this.gameObject.GetComponent<Renderer>().material.mainTextureScale.x) * (m_FlipX ? -1.0f : 1.0f),
			                                                 Mathf.Abs(this.gameObject.GetComponent<Renderer>().material.mainTextureScale.y) * (m_FlipY ? -1.0f : 1.0f));
		}
		else
		{
	...
	void OnMouseOver()
	{
		...
		if (m_WebView != null /*&& !m_WebView.IsLoading*/)
		{
            ...
            {
				RaycastHit HitInfo;
				if (GetComponent<Collider>().Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out HitInfo, 10000.0f))
				{			
